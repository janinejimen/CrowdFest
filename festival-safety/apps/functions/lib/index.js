...
exports.upsertProfile = exports.getEmergencyProfile = exports.joinWithCode = exports.createInvite = exports.createEvent = exports.setAccountType = exports.createUserProfile = exports.health = void 0;
...
exports.getEmergencyProfile = functions.https.onCall(async (data, context) => {
    if (!context.auth) {
        throw new functions.https.HttpsError("unauthenticated", "Login required.");
    }
    ...
    const logRef = (0, firebaseAdmin_1.db)().collection("emergencyAccessLogs").doc();
    const now = new Date();
    // Helper: write log
    const writeLog = async (success) => {
        await logRef.set({
            organizerUid,
            attendeeUid,
            eventId,
    ...
    const organizerMemberSnap = await (0, firebaseAdmin_1.db)()
    ...
    const attendeeMemberSnap = await (0, firebaseAdmin_1.db)()
        .collection("events").doc(eventId)
    ...
    const profileSnap = await (0, firebaseAdmin_1.db)().collection("profiles").doc(attendeeUid).get();
...
exports.upsertProfile = functions.https.onCall(async (data, context) => {
    if (!context.auth) {
        throw new functions.https.HttpsError("unauthenticated", "Login required.");
    }
    const uid = context.auth.uid;
    const { displayName, over18, age, emergencyContact, allergiesConditions, medications, consentToShareInEmergency, photoURL, } = data ?? {};
    if (typeof displayName !== "string" || displayName.trim().length < 1) {
        throw new functions.https.HttpsError("invalid-argument", "Name is required.");
    }
    if (consentToShareInEmergency !== true) {
        throw new functions.https.HttpsError("failed-precondition", "Consent is required to complete the profile.");
    }
    // Choose one: over18 checkbox is simplest. Allow age optionally.
    const normalizedOver18 = typeof over18 === "boolean" ? over18 : null;
    const normalizedAge = typeof age === "number" && Number.isFinite(age) && age >= 0 && age <= 120
        ? Math.floor(age)
        : null;
    const normalizedEmergencyContact = emergencyContact &&
        ...
        : null;
    const now = new Date();
    const profileRef = (0, firebaseAdmin_1.db)().collection("profiles").doc(uid);
